        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
            ...
            <div class="container">
                <div class="card">
                    ...
                    <div class="card-body">
                        @foreach($gurus as $g)
                        <form action="/guru/perbaharui_bio" method="post">
                            {{ csrf_field() }}
                            <input type="hidden" name="id" value="{{ $g->id }}"><br>
                            <div class="form-group">
                                <label for="namaGuru">Nama Lengkap</label>
                                <input type="text" class="form-control" name="name"  required="required" value="{{ $g->name }}">
                            </div>
                            <div class="form-group">
                                <label for="emailGuru">Email</label>
                                <input type="text" class="form-control" name="email"  required="required" value="{{ $g->email }}">
                            </div>
                            <button class="btn btn-outline-primary" type="submit">Ubah Biodata</button>
                            <a href="/guru/index" class="btn btn-outline-secondary">Kembali</a>
                        </form>
                        @endforeach
                    </div>
                </div>
